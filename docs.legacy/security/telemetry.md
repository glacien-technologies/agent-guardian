...
**TL;DR** — AgentGuardian ships with **essential-tier telemetry on by default** and an explicit opt-out. This page is the canonical reference for what's collected at each tier, what's never collected, how to audit it, and how to disable it.

Per the [contributing engineering standards](../contributing/engineering-standards.md), the telemetry source code is shipped *in the package* so anyone can read it before opting out or upgrading. The aggregator that runs on Glacien infrastructure is also in the package, so you can self-host the collector for testing.
...
|---|---|---|---|
| ESSENTIAL | `install_id` | `0bd84c9d-…` (UUID4) | Counting distinct installs |
| ESSENTIAL | `scan_id` | `aaaa1111…` (random hex) | Deduplication only |
| ESSENTIAL | `aivss` | `84` | The AIVSS distribution histogram |
| ESSENTIAL | `band` | `GOOD` | Band-distribution percentages |
| ESSENTIAL | `tier` | `T3` | Median scan time by tier |
| ESSENTIAL | `duration_seconds` | `82.5` | Scan duration percentiles |
| ESSENTIAL | `terminated_by` | `success` / `error` / `crash` | **Crash-free rate** |
| ESSENTIAL | `agents_count` | `9` | **Number of swarm agents that ran** |
| ESSENTIAL | `attempts_count` | `68` | **Total per-turn judged events (attempts tried)** |
| ESSENTIAL | `successes_count` | `63` | **Per-turn pass verdicts (target defended)** |
| ESSENTIAL | `findings_total` | `5` | **Threats captured (per scan)** |
| ESSENTIAL | `findings_critical/_high/_medium/_low` | `1/2/1/1` | Severity distribution |
| ESSENTIAL | `agent_version` | `1.0.0` | Crash rate per release |
| ESSENTIAL | `started_at` / `completed_at` | ISO-8601 UTC | Time-windowed aggregation |
| EXTENDED | `adapter` | `langgraph` | Adapter usage mix dashboard cell |
| EXTENDED | `target_mode` | `code` | Mode distribution |
| EXTENDED | `python_version` | `3.11` | Python × OS compatibility matrix |
| EXTENDED | `os_family` | `Darwin` / `Linux` / `Windows` | Compatibility matrix |
| EXTENDED | `arch` | `arm64` / `x86_64` | Compatibility matrix |

The four operational counts (`agents_count`, `attempts_count`, `successes_count`, `findings_total`) are exactly the "number of agents, number of attempts, successes, threats captured" set that's on-by-default. They cannot identify you — they are scan-level aggregates over an opaque scan_id.
...
Every public cell on `agentguardian.io/analytics` enforces **k ≥ 50** — the metric is suppressed (shown as `—`) unless at least 50 distinct `install_id`s contribute to that bucket. So:
...
# Read the collector source — this is what runs at telemetry.agentguardian.io.
...
|---|---|---|
| `AGENT_GUARDIAN_TELEMETRY_URL` | Where the client POSTs events | `https://telemetry.agentguardian.io/v1/events` |
...
- [Contributing — engineering standards](../contributing/engineering-standards.md) — the transparency commitments this implements.
- [Disclosure history](disclosure-history.md) — if you find a telemetry leak, report it here.
- [SECURITY.md](https://github.com/glacien-technologies/agent-guardian/blob/main/SECURITY.md) — vulnerability disclosure policy.
