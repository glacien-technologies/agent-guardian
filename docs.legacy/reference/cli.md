...
| `--no-serve`                 | off     | Disable the auto-spawn dashboard server for this scan. By default `scan` boots a short-lived `agent-guardian serve` child process so the printed dashboard URL works on the first click; `--no-serve` suppresses that spawn (the URL is still printed, but you need to run `agent-guardian serve` yourself in another terminal). Auto-spawn is also suppressed when stdout is not a TTY, when `--no-tui` is set, when `--debug-format json` is set, when `$CI=true`, or when `$AGENT_GUARDIAN_DISABLE_AUTO_SERVE=1`. |
| `--serve-grace-seconds N`    | `300`   | When auto-serve is active, keep the dashboard child alive for `N` seconds after the scan completes so you can click the URL and review the report. Defaults to 300 (5 min). Pass `0` to shut down immediately on scan completion; pass `-1` to keep the dashboard alive until you Ctrl-C the command. |
