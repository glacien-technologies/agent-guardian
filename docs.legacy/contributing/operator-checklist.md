...
> **TL;DR.** Maintainer pre-flight artefact for the v1.0.0 launch — the manual actions a human operator has to take in the order that minimises blast radius. Not user-facing operations. If you are running AgentGuardian in production day-to-day, the place you want is [Operations → Runbook](../operations/runbook.md).

The v1.0.0rc1 release-candidate is shippable from an engineering
...
- [ ] **GitHub Pages.** Verify deployment from `gh-pages` (the docs
      deployment workflow already configured this) — visit
...
- [ ] **Trusted Publisher.** <https://pypi.org/manage/account/publishing/>.
      ...
      environment `pypi`. Full step-by-step in
...
- [ ] **arXiv submission.** Once the beta cohort produces 30+ findings
      ...
      [Research → Preprint](../research/preprint.md), regenerate
      figures, and submit to arXiv `cs.CR`. Cross-list `cs.LG`.
- [ ] **Citation file.** Verify `CITATION.cff` matches the arXiv DOI
      once issued. See [Research → Cite AgentGuardian](../research/citation.md).

## 6. Public infrastructure (Glacien edge)

- [ ] **`agentguardian.io/leaderboard`** endpoint deploy. Once live,
      replace the placeholder in `cli.publish` with a real POST.
- [ ] **`badge.agentguardian.io`** shield endpoint deploy. The
...
- [ ] **`agentguardian.io`** domain — DNS, TLS, redirect from
      `agent-guardian.ai` and `agentguardian.ai` → canonical
      `agentguardian.io`.
