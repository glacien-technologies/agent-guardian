...
- **Severity** · medium / strategic — the Executive theme's WAI-ARIA 5-tab layout was the load-bearing decision in QA-023; collapsing one tab is a structural change to that contract. Worth doing because the Agents tab now reads as redundant after the QA-024 Overview redesign added the ASI radar (FIG. 2) which conveys the same per-ASI breakdown information visually + the QA-029 Findings-tab tightening makes the multi-tab story feel even more lopsided.
- **Found via** · 2026-06-01 operator review of the post-`9478554` Executive Overview + Findings tabs. The Overview's FIG. 2 ASI radar already shows per-ASI score for all 10 categories at a glance; the Agents tab's `_asi_rows.html` repeats the same breakdown as a vertical list with per-row severity bars. Operator: "remove the agents tab altogether we don't need it."
- **Symptom** · 5 tabs on the tab bar (Overview / Findings / Probes / Agents / Logs); the Agents tab restates information already visible on Overview as the ASI radar; the tab adds a click-cost without adding insight.
- **Expected** · 4-tab layout: Overview / Findings / Probes / Logs. The Agents tab is removed end-to-end (markup, route, tests, JS guard). The ASI breakdown stays available in the Overview tab via the radar; deeper per-ASI evidence is reachable from the Findings tab's per-bucket sections (the `#exec-sev-{key}` jump anchors).
- **Root cause hypothesis** · The 5-tab layout was specced before the Overview redesign added the ASI radar (QA-024). At spec time, the Agents tab was the only surface that showed per-ASI scores. Post-QA-024 it became redundant; nobody flipped the decision.
- **Fix area** · Five coordinated changes:
  1. **`src/agent_guardian/server/templates/dashboard/executive/layout.html`** — remove the `{% include "dashboard/executive/_tab_agents.html" %}` line from the main content area.
  2. **`src/agent_guardian/server/templates/dashboard/executive/_tab_bar.html`** — remove the Agents tab button (the `<button role="tab" aria-controls="tabpanel-agents" id="tab-agents">Agents</button>` line). Update any tab-count attribute on the `<div role="tablist">` if one exists. Net tab count: 5 → 4.
  3. **Delete `_tab_agents.html`** — the partial itself is no longer referenced anywhere. Same for the `_asi_rows.html` partial it includes IF that partial is not also used by another surface (verify via `grep -r '_asi_rows.html' src/`). If the partial IS only used by the Agents tab, delete both. If `_asi_rows.html` is also used by the Overview radar implementation (it isn't — Overview uses `_asi_radar.html`, separate), leave it alone for now and let the dead-code linter catch it later.
  4. **`executive_tabs.js`** — the JS handles tab activation via WAI-ARIA `role="tab"` selectors. Removing the Agents tab DOM removes the JS handler subscription automatically. No JS change required IF the JS doesn't have a hardcoded list of tab slugs. Verify: `grep -n 'tab-agents\|tabpanel-agents\|agents' src/agent_guardian/server/static/executive_tabs.js` — should return zero substantive matches; if any exist, remove them.
  5. **Tests** — update every test that asserts the 5-tab layout to assert 4:
     - `test_executive_tabs_render_all_five_tabs` → `test_executive_tabs_render_all_four_tabs` (assert tab count == 4, list contains Overview / Findings / Probes / Logs, does NOT contain Agents).
     - `test_executive_agents_tab_renders_asi_rows_partial` — delete (the tabpanel no longer exists).
     - Any other test that grep matches `tabpanel-agents` / `tab-agents` / `_tab_agents` / `_asi_rows` (in the Agents context): delete or rewrite.
...
## QA-029 — Findings tab UX: remove the duplicated `Findings by severity` chart at the top of the tab; make the per-finding `N evidence events` outer drawer read as an obvious click-to-expand button (chevron + hover state) instead of a styled inline link; remove the Reproducibility receipt from BOTH the Findings tab AND the Logs tab
...
- **Scope** · Three concrete sub-asks:
  ...
  3. **Remove the Reproducibility receipt from BOTH the Findings tab AND the Logs tab.** Same idiom as the QA-026 fix that removed it from the Agents tab — the receipt currently appears at the bottom of every data tab (Overview / Findings / Probes / Logs) per the QA-026 refactor. On Findings it sits below a long list of finding cards (often 15+) and reads like "is this another finding?". On Logs it sits below the scrolling log tail and competes with the scan-log content for visual focus. Per 2026-06-01 operator follow-up: remove from BOTH. The canonical reproducibility surfaces become Overview + Probes only.

     Implementation: in `src/agent_guardian/server/templates/dashboard/executive/_tab_findings.html` AND `_tab_logs.html`, remove the trailing `{% include "dashboard/executive/_reproducibility.html" %}` line just above each tab's `</section>` closer. Update the QA-026-era `test_executive_reproducibility_renders_in_each_data_tab` assertion from `body.count('data-component="reproducibility"') == 4` → `== 2` (Overview + Probes only). Add `test_executive_findings_tab_omits_reproducibility` AND `test_executive_logs_tab_omits_reproducibility` to lock both negative assertions explicitly so a future regression can't silently re-add the include to either tab.
...
- **Acceptance** ·
  ...
  5. Reproducibility receipt no longer renders inside `#tabpanel-findings` OR `#tabpanel-logs`. The receipt count across the rendered HTML drops from 4 to 2 (Overview + Probes only); Agents stays excluded (per QA-026); Findings + Logs now also excluded (this QA).
