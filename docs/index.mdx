...
Python 3.10–3.13 on Linux or macOS. Windows is community-supported. No native compilation.
...
3. **Parallel attack** — up to 14 attackers (10 ASI specialists + 4 OWASP-LLM specialists) run concurrently. Each picks probes from its lane in the bundled corpus.
...
5. **Finalise** — a deterministic AIVSS score is computed, the report is signed (HMAC-SHA256 + Ed25519), and the dashboard URL stays live for 5 minutes.
...
See [Open vs Enterprise](/concepts/open-vs-enterprise) for the full breakdown, or visit [Glacien](/enterprise) for Enterprise inquiries.
