...
3. **Attack Swarm** — up to fourteen specialist agents, each owning one
...
2026 category) plus the four OWASP-LLM specialists when
...
When an operator passes `--target-goal "exfiltrate PII"` a Commander LLM
...
goal-specific scenarios from. Without `--target-goal` the Commander step is
...
- [Adversarial swarm](/concepts/adversarial-swarm) — the fourteen
