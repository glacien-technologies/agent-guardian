...
The fourteen specialist agents (ten OWASP ASI categories + four
OWASP-LLM specialists when `--include-m2-agents` is set). They run
...
Five emitters derive their output from the same canonical
`scan.json`:
...
The local **Live dashboard** at `http://127.0.0.1:7474/scans/<scan_id>`
