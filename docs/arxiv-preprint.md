...
*v0.1 draft pending soft-beta data — figures and concrete scores will be populated after the 10-team beta wave concludes.*
...
*Results section is a placeholder pending soft-beta data — the v1.0.0
release-candidate (this draft) is the snapshot just before the first
external scans land. We will fill in:*

* Per-target AIVSS distribution across tiers.
* Per-ASI category pass-rates.
* Strategy-level comparison: which prompt-strategy module produced the
  highest finding rate on each ASI category.
...
*DRAFT — Glacien Pte. Ltd., 2026-05-27. This preprint will be revised
