...
Commands:
  ...
  suite         Run a fleet of scans in parallel from one YAML file.
  ...
  calibrate     Calibrate a judge model against the packaged calibration set.
  comment       Upsert an AgentGuardian summary comment on the current PR / MR.
  code-insights Publish a Code Insights report for a scan (Bitbucket).
  agentdojo     AgentDojo benchmark adapter (run a suite against a target).
```
...
| Flag | Default | Purpose |
| --- | --- | --- |
| `--output FORMAT` | `json` | Report format: `json` \| `sarif` \| `junit` \| `md` \| `gitlab` \| `pdf`. All six work after a stock `pip install agent-guardian` — no extras required. |
