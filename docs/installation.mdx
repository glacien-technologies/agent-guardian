...
A single `agent-guardian` CLI plus the Python SDK. The base install ships
...
<Note>
  ...
  doctor` will report `bedrock: ready` once both the `[aws]` extra and
...
- [Quickstart](/quickstart) — five minutes from `pip install` to your first AIVSS score.
- [Your first scan](/first-scan) — deeper walkthrough against a deliberately-vulnerable LangGraph target.
- [Contributing](/contributing) — set up the full dev environment with `uv sync --all-extras`.
