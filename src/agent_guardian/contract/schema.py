...
def _validate_jinja_template(template: str) -> str:
    ...
    env = Environment(autoescape=False)  # nosec B701  # nosemgrep: python.jinja2.security.audit.autoescape-disabled-false.incorrect-autoescape-disabled,python.flask.security.xss.audit.direct-use-of-jinja2.direct-use-of-jinja2  # codeql[py/jinja2/autoescape-false] — AST parse only via env.parse() + meta.find_undeclared_variables(); template is NEVER rendered, never reaches a browser, autoescape is meaningless for AST walking
    try:
